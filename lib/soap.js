...
const { isObject, formatArray } = require('./util');
...
module.exports = class Soap {
    ...
    async retrieveBulk(type, props, requestParams) {
         ...
         * @param {string} type -SOAP Object type
         * @param {Object} objects - Objects and values to be created
         * @param {Object} options - configuration of the request
         * @returns {Promise<Object>} SOAP object converted from XML
         */
    async create(type, objects, options) {
            const body = {
                CreateRequest: {
                    '@_xmlns': 'http://exacttarget.com/wsdl/partnerAPI',
                    Options: options,
                    Objects: await _formatPayloadArray(objects),
                },
            };
         ...
         * @returns {Promise<Object>} SOAP object converted from XML
         */
    async update(type, objects, options) {
            const body = {
                UpdateRequest: {
                    '@_xmlns': 'http://exacttarget.com/wsdl/partnerAPI',
                    Options: options,
...
/**
 * 
 * @param {Object} obj Payload data for Create/Update requests
 * @returns {Promise<Object>} Formatted payload for XML Request Envelope
 */
async function _formatPayloadArray(obj) {
    let orgObj = obj;
    const formatted = await _formatArray(obj, orgObj)
    return formatted
}
 ...
 * 
 * @param {Object} obj Payload data for Create/Update requests
 * @param {Object} orgObj Object reference for returned payload
 * @returns {Promise<Object>} Modified payload data for XML Request
 */
async function _formatArray(obj, orgObj) {
    for (let a in obj) {
        if (typeof obj[a] === 'object') {
            if (!Array.isArray(obj[a])) {
                await _formatArray(obj[a])
            } else {
                let key;
                switch (a) {
                    case 'Fields':
