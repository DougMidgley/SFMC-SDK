...
import { isObject, SOAPError, NetworkError } from './util.js';
...
export default class Soap {
    ...
    async _apiRequest(requestOptions) {
        ...
        try {
            _url = new URL('/Service.asmx', this.auth.authObject.soap_instance_url);
            const _requestOptions = {
                method: 'POST',
                headers: Object.assign(requestOptions.headers || {}, {
                    'Content-Type': 'text/xml',
                    SOAPAction: requestOptions.action,
                }),
                body: _buildEnvelope(requestOptions.body, this.auth.authObject.access_token),
            };
            ...
            payloadResponse = await apiResponse.text();
        } catch (error) {
        ...
        try {
            ...
            return await _parseResponse(apiResponse, payloadResponse, requestOptions.key);
        } catch (error) {
            this.options.requestAttempts--;
            ...
            //     throw new NetworkError(error, _url);
            //     //unknown error
            //     // throw new SOAPError(error, payloadResponse, undefined);
            // }
        }
    }
}
 ...
 *
 * @param {object} requestBody Object form of the payload
 * @param {string} token access token for authentication
 * @returns {string} XML string payload
 */
function _buildEnvelope(requestBody, token) {
 ...
 *
 * @param {object} apiResponse reponse object from API
 * @param {object} payloadResponse payload including whole SOAP response
 * @param {string} key key of the expected response body
 * @returns {Promise.<any | Error>} Result of request in Object format
 */
async function _parseResponse(apiResponse, payloadResponse, key) {
    const xmlToJson = new XMLParser({ ignoreAttributes: true });
    const soapBody = xmlToJson.parse(payloadResponse)?.['soap:Envelope']?.['soap:Body'];
    if (soapBody?.[key]) {
        ...
        if (
            ...
            throw new SOAPError(apiResponse, soapBody[key]);
        }
        return soapBody[key];
    }
    // something else went wrong (for example bad request) returning parsed xml
    throw new SOAPError(apiResponse, soapBody || payloadResponse);
}
/**
