...
module.exports = class Soap {
    ...
    retrieve(type, props, requestParams) {
        if (!type) throw new Error('Retrieve requires a type');
    ...
    create(type, props, requestParams) {
        if (!type) throw new Error('Create requires a type');
    ...
    update(type, props, requestParams) {
        if (!type) throw new Error('Update requires a type');
        if (Object.keys(props).length == 0)
    ...
    delete(type, props, requestParams) {
        if (!type) throw new Error('Delete requires a type');
        if (Object.keys(props).length == 0)
    ...
    describe(type) {
        return _apiRequest(
        if (!type) throw new Error('Describe requires a type');
            this.auth,
            {
                action: 'Describe',
                req: {
    ...
    execute(type, props) {
        return _apiRequest(
        if (!type) throw new Error('Execute requires a type');
            this.auth,
            {
                action: 'Execute',
                req: {
    ...
    perform(type, action, payload) {
        if (!type) throw new Error('Peform requires a type');
        if (!payload)
