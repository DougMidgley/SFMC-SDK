'use strict';
import { RestError, isJSONResponse } from './util.js';
    ...
    async getAccessToken(forceRefresh) {
        if (Boolean(forceRefresh) || _isExpired(this.authObject)) {
            try {
                ...
                error.endpoint = this.authObject?.auth_url;
                throw error;
            }
...
/**
 * @param {import('./index.js').AuthObject} authObject Auth object
...
/**
 * @param {import('./index.js').AuthObject} authObject Auth Object for api calls
...
async function _requestToken(authObject) {
    ...
    const apiResponse = await fetch(new URL('/v2/token', authObject.auth_url), {
        method: 'post',
    ...
    const payloadResponse = isJSONResponse(apiResponse)
        ? await apiResponse.json()
        : await apiResponse.text();
    if (!apiResponse.ok) {
        throw new RestError(apiResponse, payloadResponse);
    }
    return Object.assign(authObject, payloadResponse, {
        expiration: process.hrtime()[0] + payloadResponse.expires_in,
    });
}
/**
