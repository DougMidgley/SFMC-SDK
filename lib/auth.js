...
export default class Auth {
     ...
     * @param {string} authObject.grant_type Grant Type. If present then access_token should be passed to the Auth object.
    ...
    constructor(authObject, options) {
        if (!authObject) {
            ...
            authObject.grant_type == GRANT_TYPE_AUTHORIZATION_CODE
        ) {
    ...
    async getAccessToken(forceRefresh, remainingAttempts) {
        if (
            !(
                this.authObject &&
                this.authObject.grant_type &&
                this.authObject.grant_type === GRANT_TYPE_AUTHORIZATION_CODE
            )
        ) {
