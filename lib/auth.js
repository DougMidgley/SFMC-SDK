...
export default class Auth {
    ...
    constructor(authObject, options) {
        ...
        if (typeof authObject.client_id !== 'string') {
            throw new TypeError('client_id or client_secret must be strings');
        }
        if (!authObject.client_secret) {
            throw new Error('client_id or client_secret is missing or invalid');
        }
        if (typeof authObject.client_secret !== 'string') {
            throw new TypeError('client_id or client_secret must be strings');
        }
        if (!authObject.account_id) {
            throw new Error('account_id is missing or invalid');
        }
        const accountId =
            'string' === typeof authObject.account_id
                ? Number.parseInt(authObject.account_id)
        ...
        if (!authObject.auth_url) {
            throw new Error('auth_url is missing or invalid');
        }
        if (typeof authObject.auth_url !== 'string') {
        ...
        if (authObject.scope && getInvalidScopes(authObject.scope).length > 0) {
            throw new Error(
...
/**
 * @param {AuthObject} authObject Auth object
...
/**
 * @param {AuthObject} authObject Auth Object for api calls
 * @returns {Promise.<any>} updated Auth Object
 */
