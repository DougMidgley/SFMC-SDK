...
export default class Rest {
    ...
    async _apiRequest(requestOptions) {
        ...
        const _url = new URL(requestOptions.url, this.auth.authObject.rest_instance_url);
        let apiResponse;
        let payloadResponse;
        try {
            await this.auth.getAccessToken();
            ...
            if (this.options?.eventHandlers?.logRequest) {
                this.options.eventHandlers.logRequest(_requestOptions);
            }
