...
module.exports = class Rest {
    ...
    async getBulk(url, pageSize) {
        ...
        queryParameters.set('$pageSize', Number(pageSize || 50).toString());
        do {
            queryParameters.set('$page', Number(page).toString());
            const temporary = await this._apiRequest(
                {
                    method: 'GET',
            ...
            if (Array.isArray(temporary.items)) {
                iteratorField = 'items';
            } else if (Array.isArray(temporary.definitions)) {
                iteratorField = 'definitions';
            } else {
                throw new TypeError('Could not find an array to iterate over');
            }
            if (collector && Array.isArray(temporary[iteratorField])) {
                collector[iteratorField].push(...temporary[iteratorField]);
            } else if (collector == undefined) {
                collector = temporary;
            }
            if (
                Array.isArray(collector[iteratorField]) &&
                collector[iteratorField].length >= temporary.count &&
                (!isTransactionalMessageApi ||
                    (isTransactionalMessageApi && temporary.count != temporary.pageSize))
            ) {
