...
export default class Rest {
    ...
    ['_isTransactionalMessageApi'](url) {
        return Boolean(url) && this.transactionalApis.some((api) => url.includes(api));
    }
    /**
     ...
     */
    ['_isLegacyApi'](url) {
        return Boolean(url) && url.startsWith('/legacy/v1/');
    }
    /**
     ...
     */
    async *['_iterateBulkPages'](
        url,
        pageSize = 50,
        iteratorField,
        shouldEmitOnLoop = false,
        ...
        const queryParameters = new URLSearchParams(url.split('?', 2)[1]);
        let collector;
        ...
        try {
            do {
                ...
                const mergedLength = shouldAccumulate ? collector[iteratorField].length : accumulatedLength;
                ...
                yield {
                    ...
                    responseBatch,
                    collector: shouldAccumulate ? collector : undefined,
                };
                ...
                if (isTransactionalMessageApi && responseBatch[countKey] === pageSize) {
                    page++;
                    shouldPaginate = true;
                    if (shouldEmitOnLoop && this.options?.eventHandlers?.onLoop) {
                        this.options.eventHandlers.onLoop(undefined, collector?.[iteratorField]);
                    }
                } else if (
                    responseBatch[iteratorField].length < pageSize ||
                    mergedLength >= responseBatch[countKey]
                ) {
                    // stop when the page is short or the reported total has been retrieved
    ...
    async ['_apiRequest'](requestOptions, remainingAttempts, headers = {}) {
        ...
        try {
            ...
            const requestError = /** @type {import('./util.js').EnhancedRestError} */ (error);
            ...
            if (remainingAttempts && requestError.response?.status === 401) {
                await this.auth.getAccessToken(true);
                return this._apiRequest(requestOptions, 1);
            }
            throw new RestError(requestError);
        }
        ...
        try {
            ...
            if (requestOptions.url) {
                ...
                const pathPart = requestOptions.url.startsWith('/')
                    ...
                    : requestOptions.url;
                requestError.endpoint = base ? base + pathPart : pathPart;
            }
            if (
 ...
 *
 * @param {Object.<string, any>} options API request opptions
 */
