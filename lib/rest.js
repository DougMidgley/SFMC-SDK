...
module.exports = class Rest {
     ...
     * @param {string} [iteratorField] attribute of the response to iterate over ('items'|'definitions'|'entry')
    ...
    async getBulk(url, pageSize, iteratorField) {
        ...
        queryParameters.set(pageSizeName, Number(pageSize || 50).toString());
        do {
            queryParameters.set(pageName, Number(page).toString());
            ...
            if (!iteratorField) {
                if (Array.isArray(responseBatch.items)) {
                    iteratorField = 'items';
            ...
            if (
                ...
                collector[iteratorField].length >= responseBatch[countName] &&
                (!isTransactionalMessageApi ||
                    (isTransactionalMessageApi &&
                        responseBatch[countName] != responseBatch[pageSizeName]))
            ) {
